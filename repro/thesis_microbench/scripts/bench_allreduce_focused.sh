...
# Inter: NO --cudagraph. The perftest's RUN_RDXN_GRAPH path captures
# `iters + warmup_iters` calls into a single CUDA graph and launches that
# graph TWICE (warmup_launch + timed_launch — see coll_test.h:225-239),
# so cross-node 1 GiB at ~36 s/call × (5+2)*2 = 504 s per size, all sizes
# combined > 1500 s slurmstep timeout → bench hangs without producing data.
# Without --cudagraph the path is RUN_RDXN_STREAM: just `iters + warmup_iters`
# launches per size with no extra factor → ~17 min for full sweep, completes.
HOST_ARGS_INTER="-b 128 -e 2147483648 -n 5 -w 2 -d float -o sum"
